/* Name: Pete, the baker
 * Level: 5kyu
 * Instructions: Pete likes to bake some cakes. He has some recipes and ingredients.
 ...
 * of flour or 200 g of sugar are simply 1 or 200). Ingredients that are not present
...
function cakes(recipe, available) {
  const hasIngredients = Object.keys(recipe).every(key => available.hasOwnProperty(key));
  if (!hasIngredients) {
    return 0;
  }
  ...
  Object.keys(recipe).forEach(key => {
    const curCount = parseInt(available[key] / recipe[key]);
