 ...
 * Level: 6kyu
 * Instructions: A Narcissistic Number is a number which is the sum of its own digits,
...
function narcissistic(value) {
  return value === value.toString().split('').map(Number).reduce((acc, cur, idx, arr) => acc + Math.pow(cur, arr.length), 0);
}
