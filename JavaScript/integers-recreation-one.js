 ...
 * Level: 5kyu
 * Instructions: Divisors of 42 are : 1, 2, 3, 6, 7, 14, 21, 42. These divisors
 * squared are: 1, 4, 9, 36, 49, 196, 441, 1764. The sum of the squared divisors
...
function listSquared(start, end) {
  const getDivisors = (num) => {
            ...
            for (let i = 1; i <= Math.sqrt(num); i++)
              if (num % i === 0)
                i === Math.sqrt(num) ? divs.push(i) : divs.push(i, num / i);
            return divs;
  };

  const sumPowerDivisors = (num) => getDivisors(num).reduce((sum, curr) => sum + Math.pow(curr, 2));

  let res = [];

  for (let i = start; i <= end; i++) {
    let sum = sumPowerDivisors(i);
