/* Name: Pick peaks
 * Level: 5kyu
 * Instructions: In this kata, you will write a function that returns the positions and the values
...
const pickPeaks = array => array.map((val, idx) => ({val, idx}))
                                .filter((_, idx, arr) => idx > 0 && arr[idx].val !== arr[idx-1].val)
                                .filter((_, idx, arr) => idx > 0 && idx < arr.length - 1 &&
                                                         arr[idx-1].val < arr[idx].val && arr[idx].val > arr[idx+1].val)
                                .reduce((acc, cur) => ({pos:[...acc.pos, cur.idx], peaks:[...acc.peaks, cur.val]})
                                        , {pos:[], peaks:[]});
