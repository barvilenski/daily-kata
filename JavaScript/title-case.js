/* Name: Title Case
 * Level: 6kyu
 * Instructions: A string is considered to be in title case if each word in the
 * string is either (a) capitalised (that is, only the first letter of the word
...
function titleCase(title, minorWords) {
  ...
  const exceptions = (minorWords === '' || minorWords === undefined) ? [] : minorWords.toLowerCase().split(' ');

  return words.map((word, idx) => exceptions.includes(word) && idx != 0 ? word : word[0].toUpperCase() + word.slice(1)).join(' ');
}
