/* Name: Tail Swap
 * Level: 7kyu
 * Instructions: You'll be given a list of two strings, and each will contain exactly
 * one colon in the middle (but not at beginning or end). The length of the strings,
...
const tailSwap = arr => {
  let res = [];
  res.push(arr[0].split(':')[0] + ':' + arr[1].split(':')[1]);
  res.push(arr[1].split(':')[0] + ':' + arr[0].split(':')[1]);
  return res;
};
