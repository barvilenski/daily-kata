 ...
 * Level: 5kyu
 * Instructions: My friend John and I are members of the "Fat to Fit Club (FFC)".
...
function orderWeight(str) {
  ...
  return str.split(' ')
         .map((num) => ({
                        weight: num,
                        newWeight: num.split('').map(Number).reduce((acc, cur) => acc + cur)
                       }))
         .sort((a, b) => a.newWeight === b.newWeight ? a.weight.localeCompare(b.weight) : a.newWeight - b.newWeight)
         .map((weights) => weights.weight)
         .join(' ');
}
